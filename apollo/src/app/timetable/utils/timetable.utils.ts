...
export class TimetableUtils {
   ...
   public static getDisplayableHours(semester?: Semester): Interval {
      ...
      semester.activities.forEach(activity => {
         ...
         const excessMinutes = (activity.time.startingMinute + activity.time.length) % 60;
         const endingHour = activity.time.startingHour + Math.floor((activity.time.startingMinute + activity.time.length) / 60) - Number(!excessMinutes);
